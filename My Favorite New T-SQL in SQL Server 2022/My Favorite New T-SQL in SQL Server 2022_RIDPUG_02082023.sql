/*	My Favorite New T-SQL in SQL Server 2022
	...
	Created and tested using SQL Server 2022 RTM (16.0.1000.6 - released 11/6/2022)
...
SELECT
	*
FROM Sales.Orders
WHERE PickedByPersonID <> 3; -- Does not include NULL.  Incurs ANSI_NULLS risk.
-- Returns 59,390 rows
SELECT
	*
FROM Sales.Orders
WHERE ISNULL(PickedByPersonID, 3) <> 3 ; -- Does not include NULL.  Does not incur ANSI_NULLS risk, but execution plan will have trouble with it.
-- Returns 14,205 rows
SELECT
	*
FROM Sales.Orders
...
SELECT
	*
FROM Sales.Orders
WHERE PickedByPersonID IS NOT DISTINCT FROM 3; -- Only returns values of 3.  Considers NULL to be different from 3 and will not return NULL.
...
-- With no filter, this is relatively efficient, even if it may not appear to be.
SELECT
...
SELECT
	16, -- 10000 (16)
	LEFT_SHIFT(16, 1), -- 100000 (32)
	LEFT_SHIFT(16, 2); -- 1000000 (64)

SELECT
	16, -- 10000 (16)
	RIGHT_SHIFT(16, 1), -- 1000 (8)
	RIGHT_SHIFT(16, 2), -- 1000 (8)
	RIGHT_SHIFT(16, 5); -- Right shifting bits past the decimal point removes them, therefore this is now zero.

SELECT
	BIT_COUNT(16), -- 10000 (16)
	BIT_COUNT(15), -- 1111 (15)
	BIT_COUNT(1), -- 1
	BIT_COUNT(0); -- 0
...
*****************************DATE_BUCKET()*********************************************
***************************************************************************************/
DECLARE @datetime2 DATETIME2(3) = '8/29/2022 17:25:01.196';
SELECT
...
DECLARE @datetime2 DATETIME2(3) = '8/29/2022 17:25:01.196';
DECLARE @origin DATETIME2(3) = '1/1/2022 00:00:00';
DECLARE @origin1 DATETIME2(3) = '1/1/2022 01:00:00';
DECLARE @origin2 DATETIME2(3) = '1/1/2022 02:00:00';
DECLARE @origin3 DATETIME2(3) = '1/1/2022 03:00:00';
SELECT
...
SELECT
	value
FROM GENERATE_SERIES(1, 1100000000, 1000000); -- Start at 1, End at 1100000000, increment by 1000000

-- Second parameter is an INT
SELECT
	value
